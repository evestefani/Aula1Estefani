...
namespace ExercicioForWhileProfessor
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Informe a busca da palavra:");
            var wordCount = FindByWhile(Console.ReadLine());
            Console.WriteLine("Achados:" + wordCount);
        ...
        /// <returns>Retorna a quantidade encontrada de texto</returns>

        private static int FindByWhile(string wordFind)
        {

            var contetToFind = TextContent();
            var lengthText = contetToFind.Length - 2;
            var countWordFind = 0;
            int i = 0;

            while (i < lengthText)
            {
                var compareWord = contetToFind[i].ToString() +
                    contetToFind[i + 1].ToString() +
                    contetToFind[i + 2].ToString();
