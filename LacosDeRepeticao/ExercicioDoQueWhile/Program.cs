    ...
    class Program
    {
        static void Main(string[] args)
        {
         ...
         Percebemos, cada vez mais, que o julgamento imparcial das eventualidades talvez venha a ressaltar a relatividade das regras de conduta normativas.Por outro lado, a execução dos pontos do programa estimula a padronização das condições financeiras e administrativas exigidas. O empenho em analisar a crescente influência da mídia acarreta um processo de reformulação e modernização dos relacionamentos verticais entre as hierarquias.";
            ...
            var palavra = Console.ReadLine();//criar variavel para encontrar a palavra em questao
            var quantidade = 0; //quantidade da palavra a ser encontrada, o zero representa o inicio da contagem, inicio do texto
            var i = 0; 

            while (i < nome.Length - 2) // enquanto:  inicio for menor que o texto, o -2 vem para balancear o numero de casas apos e antes do zero
                //abaixo tivemos duas casas a frente do ponto inicial entao colocamos o -2 para equilibrar
            {
                var palavraComparar = //ToString significa para sequenciar, cada caractere em questao precisa de uma sequencia como abaixo para o numero de caracteres da palavra escolhida
                    nome[i].ToString() +
                    nome[i + 1].ToString() +
                    nome[i + 2].ToString();
                i++; //para contar um por um a palavra em questao
                if (palavraComparar == palavra) //para dar o numero correto de palavras
                {
                    quantidade++; //para contar um por um a quantidade da palavra em questao
            ...
            Console.WriteLine($"Quantidade total de '{palavra}' {quantidade}"); //ele vai dar no comando a palavra da pesquisa e a quantidade que havia no texto
            Console.ReadKey();
